...
from models.FusionEvaluator import FusionEvaluator
...
def load_datasets(hparams):
  if hparams.eval_datatype == 'imaging':
    train_dataset = ImageDataset(hparams.data_train_eval_imaging, hparams.labels_train_eval_imaging, hparams.delete_segmentation, hparams.eval_train_augment_rate, hparams.img_size, target=hparams.target, train=True, live_loading=hparams.live_loading, task=hparams.task)
    val_dataset = ImageDataset(hparams.data_val_eval_imaging, hparams.labels_val_eval_imaging, hparams.delete_segmentation, hparams.eval_train_augment_rate, hparams.img_size, target=hparams.target, train=False, live_loading=hparams.live_loading, task=hparams.task)
...
def evaluate(hparams, wandb_logger):
  ...
  if hparams.weighted_sampler:
    ...
    weights_list = [hparams.weighted_sampler[int(l)] for l in train_dataset.labels]
  ...
  logdir = create_logdir('eval', hparams.resume_training, hparams.run_name_set, wandb_logger)

  if hparams.task == 'regression':
    model = Evaluator_Regression(hparams)
  elif hparams.fusion:
    model = FusionEvaluator(hparams)
  else:
  ...
  callbacks.append(EarlyStopping(monitor=f'eval.val.{hparams.eval_metric}', min_delta=0.0002, patience=10, verbose=True, mode=mode, check_on_train_epoch_end=True))
  if hparams.use_wandb:
    callbacks.append(LearningRateMonitor(logging_interval='epoch'))

  trainer = Trainer.from_argparse_args(
    hparams, gpus=hparams.gpus, 
    callbacks=callbacks, 
    logger=wandb_logger, 
  ...
  if hparams.test_and_eval:
    if hparams.eval_datatype == 'imaging':
      test_dataset = ImageDataset(hparams.data_test_eval_imaging, hparams.labels_test_eval_imaging, hparams.delete_segmentation, 0, hparams.img_size, target=hparams.target, train=False, live_loading=hparams.live_loading, task=hparams.task)
    ...
    if hparams.eval_metric == 'auc':
      return model.best_val_score, model.auc_test.compute()
    elif hparams.eval_metric == 'acc':
      return model.best_val_score, model.acc_test.compute()