...
class ContrastiveImagingAndTabularDataset(Dataset):
  ...
  def one_hot_encode(self, subject: torch.Tensor) -> torch.Tensor:
    ...
    out = torch.cat(out)  # len(out) = num_features - num_categorical_features * (num_categories-1)
    return out
  ...
  def generate_imaging_views(self, index: int) -> List[torch.Tensor]:
    ...
    x = im.shape[-1]
    if x != self.img_size:
        im = F.interpolate(im.unsqueeze(0), size=(self.img_size, self.img_size), mode='bicubic', align_corners=False).squeeze(0)
