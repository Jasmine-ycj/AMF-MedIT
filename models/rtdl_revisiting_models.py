...
class FTTransformerBackbone(nn.Module):
    ...
    **Examples**
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.output_embed is not None:
...
class ModifiedFTTransformer(nn.Module):
    """
    ...
    def __init__(
        ...
        **backbone_kwargs,
        ...
        self.cat_embeddings = (
            CategoricalEmbeddings(n_categories, d_block, True)
            if cat_cardinalities
            else None
        )
        # <<<
    ...
    def split_cont_cat_features(self, x: Optional[Tensor]) -> List[torch.Tensor]:
