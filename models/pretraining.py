...
class Pretraining(pl.LightningModule):
  ...
  def initialize_imaging_encoder_and_projector(self) -> None:
    ...
    """
    self.encoder_imaging = ImagingEncoder(self.hparams)
    if self.hparams.model =='resnet50':
      self.pooled_dim = 2048
    elif self.hparams.model =='resnet18':
      self.pooled_dim = 512
    elif self.hparams.model in ['vit', 'resnet18_vit', 'resnet50_vit']:
      self.pooled_dim = self.hparams.vit_width
    elif self.hparams.model in ['vmamba', 'R50_ViT_B_16']:
      self.pooled_dim = 768
    else:
  ...
  def load_pretrained_tabular_weights(self) -> None:
    """
    Can load tabular encoder with pretrained weights from previous checkpoint/run
    """
    loaded_chkpt = torch.load(self.hparams.tabular_pretrain_checkpoint)
    state_dict = loaded_chkpt['state_dict']
    state_dict_encoder = {}
    for k in list(state_dict.keys()):
      if k.startswith('encoder_tabular.'):
        state_dict_encoder[k[len('encoder_tabular.'):]] = state_dict[k]
    _ = self.encoder_tabular.load_state_dict(state_dict_encoder, strict=True)
    print("Loaded tabular weights")
    if self.hparams.pretrained_tabular_strategy == 'frozen':
      for _, param in self.encoder_tabular.named_parameters():
        param.requires_grad = False
      parameters = list(filter(lambda p: p.requires_grad, self.encoder_tabular.parameters()))
      assert len(parameters)==0
  ...
  def forward_tabular(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    """
