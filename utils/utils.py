...
class PrintTransform:
    ...
    def __call__(self, x):
        print(self.message, x.size if isinstance(x, Image.Image) else x.shape)
        return x
    
def create_logdir(name: str, resume_training: bool, run_name_set:str, wandb_logger):
  basepath = join(os.path.dirname(os.path.abspath(sys.argv[0])),'runs', name)
  run_name = wandb_logger.experiment.name
  if run_name == None:
    run_name = run_name_set
    # wandb_logger.experiment.name = 'offline_exp'
...
def grab_image_augmentations(img_size: int, target: str, crop_scale_lower: float = 0.08) -> transforms.Compose:
  ...
  if target.lower() == 'dvm':
    transform = transforms.Compose([
      ...
      transforms.RandomResizedCrop(size=(img_size,img_size), scale=(crop_scale_lower, 1.0), ratio=(0.75, 1.3333333333333333), antialias=True),
      transforms.RandomHorizontalFlip(p=0.5),
      #transforms.Resize(size=(img_size,img_size)),
      transforms.Lambda(lambda x : x.float())
    ])
  elif target.lower() == 'ol3i':
    transform = transforms.Compose([
      # PrintTransform("Before Flip"),
      transforms.RandomHorizontalFlip(),
      # PrintTransform("After Flip"),
      transforms.RandomRotation(10),
      # PrintTransform("After Rotate"),
      transforms.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5),
      # PrintTransform("After ColorJitter"),
      transforms.RandomResizedCrop(size=img_size, scale=(0.2,1), antialias=True),
      # PrintTransform("After ResizedCrop"),
      transforms.Lambda(lambda x: x.float()),
      # PrintTransform("Done"),
    ])
  else:
